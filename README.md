...
- the version in `config/nvim.deprecated` has been **frozen**
- **please check [this setup](https://codeberg.org/martialboniou/nvim) on
  Codeberg** (this version is maintained and tested.)
...
(here, `fd` & `bat` are required). Here's some useful `fzf`-based functions (or _aliases_):
...
- _normal way_ (by default using `<TAB>`, there's no menu _ie_ `NO_ALWAYS_LAST_PROMPT`):
  - `<TAB>` : next completion
...
- _menu way_:
  - `<CTRL-n>` (instead of `<TAB>`) : activate the **menu** (you can now select and **multi-select**)
    - in the menu, you can type:
      ...
      - same keys as in the _normal way_ (`<TAB>`, `<SHIFT-TAB>`...)
      - `<SPACE>` : ok too
...
- (experimental; requires the old [matcher](https://github.com/burke/matcher))
  - `<CTRL-X><CTRL-N>` : matches files with _30 characters max_ inside `git ls-files` if possible;
  ...
  - (optional) if `activate_simple_matcher` is set:
    - `<CTRL-X><CTRL-T>` : same matcher but without menu (the lack of multi-select makes this option
      unneeded)
- `<CTRL-X>X` : complete the zsh _aliases_
...
- this setup requires `fzf`, `bat` and `git`_(!)_; on macOS, you need
...
- `fn a` is the prefix when skhd is activated on macOS; `F5` otherwise.
  ...
  but it is in conflict with the _snippet backward_ default keybinding in
  (Neo)Vim
