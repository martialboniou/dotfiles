...
From `$ZDOTDIR`, the starting point is the `.zshenv` then visit:

- `zshrc.d/` subdirectory for environment setting and functions' configuration
- `env/` subdirectory for local `path` customization (it includes `GOPATH`, `LUAPATH`...)
...
Notice that if you launch a subshell via `zsh` & Nix is installed, the `PATH`
won't be rebuilt (like if you ran `zsh --login` instead); it was required
because **the Nix `daemon.sh` is executed once per interactive instance**
(otherwise, the local `PATH` made by `$ZDOTDIR/.zshenv` (dynamically built or
*cached* after the first run) & the additional `PATH` generated by scripts in
`/etc/zshrc` and `$ZDOTDIR/.zshrc` won't have Nix *profiles* anymore; `opam` &
`rustup` `PATH` generators will be rerun so you might have everything else in
this actual `zsh` setup). So, **whenever you change any `$ZDOTDIR/env/dynamic`
file**, you must remove `$ZDOTDIR/cache/zshenv` **AND** launch another
interactive instance from a new terminal instance (or, even better, newly
created `tmux` window)

Notice that the numbers in front of file names in `$ZDOTDIR/env` &
