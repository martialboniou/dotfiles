...
this setup, `null-ls` takes care of `clang-format` instead of the embedded
...
arguments to `clang-format` easily now.
...
has been auto-generated by `hondana-dev.plugins.null-ls` and will be again
if removed):
...
to `true` in `~/.config/nvim/fnl/hondana-dev/plugins/null-ls.fnl`: by
