...
(tc alias funOrTable "| keymapFun" "| {[1]: keymapFun, [2]: keymapOptions}")

(tc type "table<string, table<string, funOrTable>>")
(local lsp-custom-keymaps
       {:n {:<leader>f [vim.lsp.buf.format {:desc "LSP Format"}]
            :gd [vim.lsp.buf.definition {:desc "LSP Go to Definition"}]
            :K vim.lsp.buf.hover
            :<leader>vws [vim.lsp.buf.workspace_symbol
                          {:desc "LSP View Workspace Symbols"}]
            ...
            :<leader>vrr [vim.lsp.buf.references {:desc "View LSP References"}]
            :<leader>vrn [vim.lsp.buf.rename {:desc "LSP Rename"}]
            ;;; ! for ergonomics: <leader> + ca = vca, rr = vrr, nn, vrn
            :<leader>ca [vim.lsp.buf.code_action
                         {:desc "View LSP Code Actions"}]
            :<leader>rr [vim.lsp.buf.references {:desc "View LSP References"}]
            :<leader>nn [vim.lsp.buf.rename {:desc "LSP Rename"}]}
...
(local config ;;
       (λ [_ opts]
         ...
         (lsp-zero.on_attach (λ [_ buffer]
                               (local default-opts {: buffer :remap false})
                               (each [mode maps (pairs lsp-custom-keymaps)]
                                 (each [key fun (pairs maps)]
                                   (let [set-key #(vim.keymap.set mode key $1
                                                                  $2)]
                                     (if (= :table (type fun))
                                         (let [opts (. fun 2)]
